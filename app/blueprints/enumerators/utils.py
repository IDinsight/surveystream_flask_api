...
class EnumeratorsUpload:
    ...
    def save_records(self, column_mapping, write_mode):
        ...
        if write_mode == "merge":
            enumerator_ids = [item[1] for item in records_to_write]
            ...
            existing_enumerator_ids = [result[0] for result in existing_enumerator]
                    ...
                    if enumerator_type == "surveyor":
                        ...
                        if hasattr(column_mapping, "location_id_column"):
                            ...
                            if "," in str(row[col_index]):
                                # Handle multiple location_ids
                                location_ids = [
                                    lid.strip()
                                    for lid in str(row[col_index]).split(",")
                                ]
        ...
        if records_to_update:
            for record in records_to_update:
                ...
                update_data = {
                    ...
                    if col not in excluded_columns and col in column_to_key_mapping
                }
                ...
                if hasattr(column_mapping, "custom_fields"):
                    for custom_field in column_mapping.custom_fields:
                        ...
                        custom_fields[custom_field["field_label"]] = record[col_index]
                # Add column_mapping to custom fields
                custom_fields["column_mapping"] = column_mapping.to_dict()
                    ...
                    if enumerator_type == "surveyor":
                        if hasattr(column_mapping, "location_id_column"):
                            ...
                            if not surveyor_location:
                                # Check if location_id has multiple values
                                if "," in str(record[col_index]):
                                    # Handle multiple location_ids
                                    location_ids = [
                                        lid.strip()
                                        for lid in str(record[col_index]).split(",")
                                    ]
                                    for location_id in location_ids:
                                        surveyor_location = SurveyorLocation(
                                            enumerator_uid=enumerator_uid,
                                ...
                                if "," in str(record[col_index]):
                                    # Delete all existing record and add new ones for multiple locations
