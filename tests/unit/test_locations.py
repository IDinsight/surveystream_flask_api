...
class TestLocations:
    ...
    def test_geo_levels_validate_hierarchy(
        ...
        assert response.json["errors"] == [
            "The hierarchy should have exactly one top level geo level (ie, a geo level with no parent). The current hierarchy has 0 geo levels with no parent.",
            "Geo level 'Mandal' references a parent geo level with geo_level_uid=5 that is not found in the hierarchy.",
            "Geo level 'State' is referenced as its own parent. Self-referencing is not allowed.",
            ...
            "Each geo level should be referenced as a parent geo level exactly once. Geo level 'Mandal' is referenced as a parent 0 times.",
        ]
