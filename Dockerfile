...
RUN apt-get update
RUN apt-get -y install python3-dev postgresql postgresql-contrib python3-psycopg2 libpq-dev gcc g++
...
WORKDIR ${HOME_DIR}
COPY flask_app ${HOME_DIR}/flask_app

VOLUME ${HOME_DIR}/flask_app
...
CMD ["gunicorn", "--chdir", "flask_app", "--timeout", "1000", "app:dod_app", "-b", "0.0.0.0:5001"]